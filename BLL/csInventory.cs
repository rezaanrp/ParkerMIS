...
namespace BLL
{
   ...
   {
        public DataTable SlPiecesProductByDateAndPieces(string Date, int   xPieces_)
        {
         return   new Connection.csConnection().SqlDataTabeReturn("SlPiecesProductByDateAndPieces", "@Date", Date, "@xPieces_", xPieces_.ToString());
       ...
       /// <returns>NAME GHETE VA MEGHDARE TOOLID AN </returns>
        ...
        public int fnStockPiecesCheck( string WorkYear,int xPieces_)
        {

            int Stc;
            DataTable dt =  new BLL.Connection.csConnection().SqlDataTabeReturn("SlStockPiecesCheck",
                                                                            
                                                                            "@WorkYear", WorkYear, "@Pieces", xPieces_.ToString());

            if (dt.Rows.Count > 0)
                Stc = (int)dt.Rows[0]["Stock"];
            else
                Stc = 0;
            return Stc;
