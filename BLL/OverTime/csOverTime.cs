...
namespace BLL.OverTime
{
    public class csOverTime
    {
        ...
        public DAL.Person.DataSet_PersonIO.sp_PAYAReportDataTable OverTimeIO(double PERNO,string DateFrom,string DateTo)
        {
            DAL.Person.DataSet_PersonIOTableAdapters.sp_PAYAReportTableAdapter adp = new
             DAL.Person.DataSet_PersonIOTableAdapters.sp_PAYAReportTableAdapter();
             return adp.GetData(DateFrom,DateTo, PERNO);
        }
        public string OverTimeIOLastEnter(double PERNO)
        {
            DAL.Person.DataSet_PersonIOTableAdapters.sp_PAYAReportDataTableAdapter adp = new
             DAL.Person.DataSet_PersonIOTableAdapters.sp_PAYAReportDataTableAdapter();
            DataTable dt = adp.GetData( PERNO);
            if (dt.Rows.Count > 0)
            {
                return dt.Rows[0]["Date_"].ToString();
            }
            else
                return "*";
        }
