...
namespace Payazob
{
    public partial class frmFoundryDetialsList : Form
    {
        public frmFoundryDetialsList()
        {
            ...
            cmb_xNumberFurnace.DataSource = csm.SlGenGroup("FURNACE");
            cmb_xNumberFurnace.DisplayMember = "xName";
            cmb_xNumberFurnace.ValueMember = "x_";
            cmb_xNumberFurnace.SelectedIndex = 0;
        ...
        private void cmb_xNumberFurnace_SelectedIndexChanged(object sender, EventArgs e)
        {
            if ( cmb_xNumberFurnace.SelectedIndex < 0)
                return;
            fer_N = Convert.ToByte((int)cmb_xNumberFurnace.SelectedIndex);
            fer_N++;
            if (dataGridView_F.Rows.Count < 1  )
                return;
