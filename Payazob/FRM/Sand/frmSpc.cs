    ...
    public partial class frmSpc : Form
    {
       ...
       DataTable dt_D;
        ...
        void GenB_L(string USL,string LSL ,float UCL, float LCL, float RBAR ,float XBARBAR )
        {
            ...
            txt_USL.Text = USL;
            txt_LSL.Text = LSL;
            //float USL_temp = Convert.tofloa;
            //float LSL_temp;
            txt_CP.Text = (System.Math.Round( (double.Parse(USL)  - double.Parse(LSL)) / (6 * (RBAR / 1.128)), 3)).ToString();

        //    MessageBox.Show((RBAR / 5.20).ToString());
          //  MessageBox.Show(XBARBAR.ToString());
            txt_CPL.Text = (
                             System.Math.Round(
                             (XBARBAR - double.Parse(LSL)) /
                             (  ( RBAR / 1.128 )    * 3) 
                             ,3)
                            ).ToString();
          ...
          //  textBox1.Text ="###"+ XBARBAR.ToString() + "###" + RBAR.ToString() + "###"
           //     + ((RBAR / 3.686)).ToString();
            ...
            if (  (double.Parse(txt_CPU.Text) - double.Parse(txt_CPL.Text) ) > 0)
