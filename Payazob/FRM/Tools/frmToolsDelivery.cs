...
namespace Payazob.FRM.Tools
{
    public partial class frmToolsDelivery : Form
    {
        ...
        {
           DAL.Tools.DataSet_Tools.SlToolsDeliveryDataTable dt_tt  = new BLL.Tools.csTools().SlToolsDelivery( User_);
            ...
            foreach (DataGridViewRow item1 in dataGridView1.Rows)
            {
                ...
                if (!item1.Selected)
                {
                    ...
                    foreach (DataRow item2 in dt_tt.Rows)
                    {
            ...
            r.GetReport = cs.GetReport(dt_tt, "crsToolsDelivery");
            r.ShowDialog();
            r.Dispose();
        }
