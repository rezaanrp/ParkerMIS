...
namespace MIS
{
    ...
    {
        public frmMoldingDownTimeType(bool EditAble, Payazob.CS.csEnum.TypeTree typ)
        {
            InitializeComponent();
            if (!EditAble)
            {
        ...
        void Generate()
        {
            ...
            {
                dt_D = cs.SelectMoldingDownTimeType(false);
      ...
      //    int idx = dt_D.Rows.IndexOf(dt_D.Select("x_ = " + treeView_DownTime.SelectedNode.Tag.ToString())[0]);
       ...
       //    RestoreTreeState(treeView_DownTime, TreeState_);
        ...
        {
            var savedExpansionState = treeView_DownTime.Nodes.GetExpansionState();
            if (typT == Payazob.CS.csEnum.TypeTree.DownTimeType)
            {
                int idx = dt_D.Rows.IndexOf(dt_D.Select("x_ = " + treeView_DownTime.SelectedNode.Tag.ToString())[0]);
            ...
            else if (typT == Payazob.CS.csEnum.TypeTree.DownTimeObject)
            {
        ...
        public CS.csEnum.TypeForm tyfrm = Payazob.CS.csEnum.TypeForm.Report; 
        private void treeView_DownTime_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            if (treeView_DownTime.SelectedNode == null)
                return;

            if (tyfrm == Payazob.CS.csEnum.TypeForm.Report)
            {
