...
namespace Payazob.FRM.Product
{
    public partial class frmPiecesProducsMachining : Form
    {
        public frmPiecesProducsMachining()
        {
            ...
            {
                HeaderText = "نام شیفت",
                DataSource = new BLL.GenGroup.csGenGroup().SlGenGroup("SHIP"),
                DisplayMember = "xName",
                ValueMember = "x_",
            ...
            DataGridViewComboBoxColumn dataGridViewColumn22 = new DataGridViewComboBoxColumn
            {
                HeaderText = "وضعیت دستگاه",
                DataSource = new BLL.GenGroup.csGenGroup().SlGenGroup("STMACHINE"),
                DisplayMember = "xName",
                ValueMember = "x_",
                Name = "xGenSTMACHINE_",
                DataPropertyName = "xGenSTMACHINE_",

                Visible = true
            };
            this.dataGridView2.Columns.Add(dataGridViewColumn22);
            ...
            {
                HeaderText = "نام قطعه",
                DataSource = pieces.mPiecesDataTable((int)CS.csEnum.GenFactoryGroupPieces.Site3),
                DisplayMember = "Piece",
                ValueMember = "x_",
        ...
        private void dataGridView2_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (((e.RowIndex >= 0)
                && (e.ColumnIndex >= 0)))
            {
                if (this.dataGridView2.Columns[e.ColumnIndex].Name == "combobox_PiecesLine1_")
                 {
                    ...
                    {
                        (this.dataGridView2["xProductOperation_", e.RowIndex] as DataGridViewComboBoxCell).DataSource = 
                            new csProductOperation().ProductOperationOpName((int?)this.dataGridView2[e.ColumnIndex, e.RowIndex].Value);
                    }
                    else
                    {
                        (this.dataGridView2["xProductOperation_", e.RowIndex] as DataGridViewComboBoxCell).DataSource = 
                            new csProductOperation().ProductOperationOpName(-1);
                    }
                }

               else if (this.dataGridView2.Columns[e.ColumnIndex].Name == "xGenSTMACHINE_")
                 {
                    this.dataGridView2["MachineName", e.RowIndex].Value = DBNull.Value;
                    if (this.dataGridView2[e.ColumnIndex, e.RowIndex].Value != DBNull.Value)
                    {
                        (this.dataGridView2["MachineName", e.RowIndex] as DataGridViewComboBoxCell).DataSource = 
                            new BLL.Machine.csMachine().mMachineBySTMACHINE((int?)this.dataGridView2[e.ColumnIndex, e.RowIndex].Value, 
        ...
        private void ShowDataGridViewDetials(int x_)
        {
            ...
            dt_D.xAvailableTimeColumn.DefaultValue = 480;
            this.generateForm_D();
