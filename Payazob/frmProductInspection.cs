...
namespace Payazob
{
    public partial class frmProductInspection : Form
    {
        ...
        private void frmProductInspection_Load(object sender, EventArgs e)
        {
         ...
         //   numericUpDown_DayOfYear.Text = new BLL.csshamsidate().DayOfYearShamsi().ToString();
        }
        private void filldatagirdview(DataTable dt)
        {
            ...
            this.dataGridView1.Columns["dvg_DefectNumber"].Width = 50;
        ...
        {
             Inv_ = new BLL.csInventory().InventoryCheck(Payazob.Properties.Settings.Default.WorkYear.Substring(0, 4) + "/01/01",
                                                             Payazob.Properties.Settings.Default.WorkYear.Substring(0, 4) + "/12/30",
        ...
        private void btn_send_Click(object sender, EventArgs e)
        {
            ...
            int t_d = int.Parse(numericUpDown_DayOfYear.Text);
            string xProductionDate = BLL.csshamsidate.ShamsiDateAndDayOfYear(numericUpDown_Year.Value, t_d);
            ...
            if (MessageBox.Show(" ایا می خواهید این اطلاعات  ارسال شود", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                de.InsertProductInspection((int)uc_cmbAuto_Pieces.SelectedValue, (int)numericUpDown_Year.Value,
                    numericUpDown_DayOfYear.Text,
                    (int)uc_cmbAuto_shift.SelectedValue, int.Parse(uc_txtControlNumbers.textWithoutcomma == null ? "0" : uc_txtControlNumbers.textWithoutcomma)
        ...
        void ShowPiecesProduct()
        {
            if (uc_cmbAuto_Pieces.SelectedValue != null && uc_cmbAuto_Pieces.SelectedIndex > 0)
            {
                int xt = int.Parse(numericUpDown_DayOfYear.Text);
                string xProductionDate = BLL.csshamsidate.ShamsiDateAndDayOfYear((int)numericUpDown_Year.Value, xt);
                DataTable dt = new BLL.csInventory().SlPiecesProductByDateAndPieces(xProductionDate, (int)uc_cmbAuto_Pieces.SelectedValue);

                lbl_ProductNumber.Text = "تعداد تولید" + " : " +  dt.Rows[0].ItemArray[0].ToString();
                Inv_ = new BLL.csInventory().InventoryCheck(Payazob.Properties.Settings.Default.WorkYear.Substring(0, 4) + "/01/01",
                                                Payazob.Properties.Settings.Default.WorkYear.Substring(0, 4) + "/12/30",
                                                Payazob.Properties.Settings.Default.WorkYear,
                                                (int?)uc_cmbAuto_Pieces.SelectedValue);
                lbl_inv.Text = " موجودی در جریان" + " : " +  Inv_.ToString();
            }
        }
        private void uc_txt_DayOfYear_TextChanged(object sender, EventArgs e)
        {
            ShowPiecesProduct();
        }

        private void uc_cmbAuto_Pieces_SelectedIndexChanged(object sender, EventArgs e)
        {
            ShowPiecesProduct();

        }

        private void numericUpDown_DayOfYear_ValueChanged(object sender, EventArgs e)
        {
            ShowPiecesProduct();
